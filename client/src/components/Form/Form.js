...
var utilRequirements = [];

const Form = ({ currentId, setCurrentId }) => {
    const [currentWidth, currentHeight] = useWindowResize();
    const [jobOfferData, setJobOfferData] = useState({ company: '', title: '', description: '', requirements: '', RAL: '' });
    ...
    if(jobOffer?.requirements) utilRequirements = jobOffer.requirements;
    ...
    const handleSubmit = (e) => {
        ...
        if(currentId) {
            dispatch(updateJobOffer( currentId, { ...jobOfferData, name: user?.result?.name, requirements: utilRequirements } ));
        } else {
            dispatch(createJobOffer({ ...jobOfferData, name: user?.result?.name, requirements: utilRequirements }));
        }
        clear();
    }
    ...
    const addRequirement = () => {
        ...
        console.log('utilRequirements(before): ', utilRequirements);
        utilRequirements.push(util);
        console.log('utilRequirements: ', utilRequirements);
    }
                            ...
                            {( !jobOffer?.requirements ) ? <Typography component="p" className={classes.marginLeft}>No requirements currently</Typography> : 
                                <Grid>
                                    {jobOffer.requirements!= [] ?
                                        jobOffer.requirements.map((skill, index) => (
                                            <Skill index={index} arrayLength={jobOffer.requirements.length} skill={skill} showDeleteIcon={true} userId={null} profileCalling={false} skillArray={utilRequirements}/>
